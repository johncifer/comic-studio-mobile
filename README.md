...
| 🧹 去黑条 | 去条带 | 🤖**自动识别**（识别阈值滑块 + 可选「圈选范围」限定区域 → 半透明蒙版预览 → 确认消除，**走电脑工作室 deband 引擎**，需填电脑局域网 IP）/ 🖌**手动消除**（图上涂白要去掉的区域 → **手机本地 Mi-GAN ONNX 推理，不连电脑**，可去任意瑕疵/文字/物体）。预览图像导演台一样大，结果可**写回** `_debanded.png`。手动模式零电脑依赖；自动模式需同一局域网 + 电脑工作室运行（防火墙放行 8787） |
...
3. **去黑条**：🤖自动识别走电脑工作室 deband 引擎（手机填电脑局域网 IP、防火墙放行 **8787**）；🖌手动消除已**改成本地 Mi-GAN ONNX 推理（onnxruntime-web）**，手机内直接跑、不连电脑、不依赖 iopaint——首次使用点「📦 加载本地模型」加载 `vendor/models/migan_512.onnx`（约 27MB），APK 已内置。
