...
| 🧹 去黑条 | 去条带 | **后端跑电脑端 iopaint（AI 修复）**：🤖自动识别（识别阈值滑块 + 可选「圈选范围」限定区域 → 半透明蒙版预览 → 确认消除）/ 🖌手动消除（图上涂白要去掉的区域 → 发送到电脑 iopaint 修补，可去任意瑕疵/文字/物体）/ ✂️裁剪黑边（纯前端离线）。预览图像导演台一样大，结果可**写回** `_debanded.png`。需手机与电脑同一局域网、电脑工作室在运行 |
...
3. **去黑条需电脑端 iopaint**：🤖自动识别与🖌手动消除都走电脑工作室（手机填电脑局域网 IP 连接），电脑关机/断网则这两模式不可用；仅「✂️裁剪黑边」是纯前端离线。防火墙需放行 **8787**（工作室）与 **7900**（iopaint）。
